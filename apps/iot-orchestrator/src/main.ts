...
const { NODE_ENV, GRPC_PORT_IOT_ORCHESTRATOR, PORT_IOT_ORCHESTRATOR } =
  process.env;

async function bootstrap() {
  const logger = new IotconLogger(
    MetaContextEnum.IOT_ORCHESTRATOR,
    NODE_ENV === 'development' ? resolve(__dirname) : undefined,
  );
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    logger,
  });
  const protoPath = join(__dirname, '../../../proto/orchestration.proto');
  ...
  await app.listen(PORT_IOT_ORCHESTRATOR);
  Logger.log(`HTTP Server is listening on port ${PORT_IOT_ORCHESTRATOR}`);

  await app.startAllMicroservices();
  Logger.log(`gRPC Server is listening on port ${GRPC_PORT_IOT_ORCHESTRATOR}`);
}
bootstrap();
